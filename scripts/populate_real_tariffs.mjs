...
async function main() {
  ...
  try {
    ...
    console.log('-'.repeat(80));
    const sortedRegions = [...regions].sort((a, b) => a.name.localeCompare(b.name, 'ru'));
    ...
    if (moscowRegion) {
      ...
      console.log('\nMOSCOW (Москва) - Sample records (first 10):');
      for (const rec of moscowRecords.slice(0, 10)) {
        console.log(`  ${rec.recorded_at} | ${rec.tariff_type.padEnd(11)} | ${rec.consumer_type.padEnd(15)} | ${rec.time_zone.padEnd(10)} | ${rec.price} руб`);
      }
      console.log(`  ... and ${moscowRecords.length - 10} more records`);
    }
    ...
    if (spbRegion) {
      ...
      console.log('\nST. PETERSBURG (Санкт-Петербург) - All records:');
